    ...
    <div className={cn("flex flex-col overflow-hidden rounded-[var(--radius-lg)] border border-border bg-surface", className)}>
      ...
      {creatingDate && (
        <div className="flex flex-wrap items-center gap-2 border-b border-border bg-warning/8 px-3 py-3 sm:px-4">
          ...
          <input autoFocus value={draftTitle} onChange={(e) => setDraftTitle(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") handleCreate(creatingDate); if (e.key === "Escape") { setCreatingDate(null); setDraftTitle(""); setDraftTime("09:00") } }} placeholder="Event title" className="h-7 flex-1 min-w-[140px] rounded-md border border-border bg-surface px-2 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-focus-ring/25" />
          <input type="time" value={draftTime} onChange={(e) => setDraftTime(e.target.value)} className="h-7 w-[110px] rounded-md border border-border bg-surface px-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-focus-ring/25" />
      ...
      {editingId && (() => {
        ...
        return (
          <div className="flex flex-wrap items-center gap-2 border-b border-border bg-surface-muted px-3 py-3 sm:px-4">
            ...
            <input autoFocus defaultValue={ev.title} onKeyDown={(e) => { if (e.key === "Enter") { const v = (e.target as HTMLInputElement).value; const tv = (document.getElementById(`cal-edit-time-${ev.id}`) as HTMLInputElement | null)?.value ?? ev.time ?? ""; if (v.trim()) handleUpdate({ ...ev, title: v.trim(), time: tv || undefined }) } if (e.key === "Escape") setEditingId(null) }} id={`cal-edit-${ev.id}`} placeholder="Title" className="h-7 flex-1 min-w-[140px] rounded-md border border-border bg-surface px-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-focus-ring/25" />
            <input type="time" defaultValue={ev.time ?? ""} id={`cal-edit-time-${ev.id}`} className="h-7 w-[110px] rounded-md border border-border bg-surface px-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-focus-ring/25" />
