...
export function Callout({ variant = "info", icon, title, className, children, ...props }: CalloutProps) {
  ...
  return (
    <div
      className={cn("flex gap-3 rounded-xl border px-4 py-3.5", wrap, className)}
      role="note"
      {...props}
    >
      ...
      <div className="flex-1 min-w-0">
        {title && (
          <p className="text-sm font-semibold text-gray-900 dark:text-white mb-0.5 leading-snug">{title}</p>
        )}
